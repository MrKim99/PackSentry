...
import { Play, Square, Video, Volume2, ShieldCheck, AlertCircle, RefreshCw, Layers, CheckSquare, Cloud, ExternalLink } from 'lucide-react';
          ...
          {!cameraActive && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6 text-center">
              ...
              <div className="max-w-md px-4">
                <p className="font-bold text-rose-450 text-sm flex items-center justify-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-rose-500 animate-ping"></span>
                  Camera đang bị Trình Duyệt hoặc iFrame Chặn!
                </p>
                <p className="text-[11px] text-slate-400 mt-2 leading-relaxed">
                  Vì lý do bảo mật của trình duyệt Chrome, Camera thiết bị và súng quét barcode rảnh tay sẽ không thể khởi chạy bên trong khung nhúng (iFrame).
                </p>
                <p className="text-[11px] text-emerald-400 mt-1 font-semibold">
                  Giải pháp cực nhanh: Hãy click nút bên dưới để chuyển ứng dụng sang Tab mới độc lập!
                </p>
                ...
                <div className="mt-4 flex flex-col sm:flex-row gap-2.5 justify-center">
                  <a 
                    ...
                    className="inline-flex items-center justify-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white font-bold py-2 px-4 rounded-xl text-xs shadow-md transition-all cursor-pointer"
                  >
                    <ExternalLink size={13} />
                    Mở tab mới & Kết nối Camera
                  </a>
                  <button 
                    onClick={() => {
                      ...
                      navigator.mediaDevices.getUserMedia({ video: true })
                        ...
                        .catch((err) => {
                          addLog('error', `Yêu cầu quyền thất bại: ${err.message}`);
                        });
                    }}
                    className="inline-flex items-center justify-center gap-1.5 bg-slate-800 hover:bg-slate-700 active:scale-95 text-slate-300 font-bold py-2 px-4 rounded-xl text-xs border border-slate-700 transition-all cursor-pointer"
                  >
